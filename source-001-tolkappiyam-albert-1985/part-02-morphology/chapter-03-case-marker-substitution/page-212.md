---
source_id: source-001-tolkappiyam-albert-1985
pdf_page: 212
printed_page: 185
part: Part II — Morphology
chapter_number: 3
chapter_tamil: வேற்றுமை மயங்கியல்
chapter_english: A Chapter on the Case-marker Substitution
language: English
status: verified
sutras: 110-114
notes: 12-13
translation_of: page-211
chapter_status: complete
---

# Tolkāppiyam in English

## Metonymy

### 110

Metonymy occurs when the whole is used for the part or the part is used for the whole, the produce is called after the place of production, or when an object is named after its quality, the produce is named after the material of which it is made and in noun compounds or when the work is called by the name of the author.

## Two ways

### 111

These may point out to objects of contiguity or distant objects by way of comparison. Since they have these qualities they must be understood clearly.

## Markers

### 112

They are to be considered very much like the case markers.

## Measures, weights

### 113

The learned tell us that names of measures and weights should be included in this list.

## Other occurrences

### 114

If there are other occurrences they may be understood by what are described here.

---

## Notes

### 12

In verses 93 to 96 instances where the case marker is not explicitly mentioned, where the zero case marker is used are referred to. They say what alternatives are possible to fill these gaps.

### 13

This does not talk about the suffixation of the case marker which was done already in verse 70. This shows the place for a case marker in a continuum, that is the place for the noun-marker combine.
