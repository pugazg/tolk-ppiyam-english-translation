# Tolkāppiyam — English Translation

A page-by-page archival transcription of **Dr. D. Albert's _Tolkāppiyam: Phonology & Morphology (An English Translation)_**, published by the International Institute of Tamil Studies in July 1985.

## Source

- Source PDF: `TVA_BOK_0013548_Tolkāppiyam.pdf`
- Physical/PDF pages: 316
- Languages: Tamil and English
- Scope: one Markdown file per physical PDF page, preserving front matter, contents, chapter order, Tamil source pages, English translation pages, notes, references and printed page numbering.

## Repository structure

```text
source-001-tolkappiyam-albert-1985/
├── README.md
├── manifest.md
├── contents-index-chapters/
│   ├── README.md
│   ├── contents.md
│   ├── chapter-index.md
│   └── remaining-chapter-ranges.md
├── front-matter/
├── part-01-phonology/
│   ├── chapters 1–9/
│   ├── research-topics/
│   ├── index/
│   └── page-175.md
└── part-02-morphology/
    ├── page-176.md
    ├── chapter-01-formation-and-distribution-of-morphemes/
    ├── chapter-02-cases/
    ├── chapter-03-case-marker-substitution/
    ├── chapter-04-conventions-of-the-vocative/
    └── chapter-05-nouns/
```

Each page file records the physical PDF page, printed page label where present, language, section and transcription status. Doubtful readings are explicitly marked rather than silently corrected.

## Progress

PDF pages **1–236** are transcribed and verified.

- **Part I — Phonology is complete**, including its nine chapters, Research Topics and Index.
- **Part II — Morphology Chapters 1–5 are complete**.

There are **4 chapters remaining**, all in Part II — Morphology. The next chapter is **வினையியல் / A Chapter on the Verbs**, beginning at PDF page **237**.

See [`manifest.md`](source-001-tolkappiyam-albert-1985/manifest.md) for progress and [`contents-index-chapters/`](source-001-tolkappiyam-albert-1985/contents-index-chapters/) for maintained navigation.
